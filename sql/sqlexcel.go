...
func sqlQuery() {
	...
	for rows.Next() {
		var each = student{}
		var err = rows.Scan(&each.id, &each.name, &each.number_of_employees, &each.job_id, &each.department_id, &each.finger_id, &each.gender, &each.place_of_birth, &each.date_of_birth)
	...
	for _, each := range result {
		...
		// xlsx.SetCellValue(sheet1Name, fmt.Sprintf("D%d", each.id), each.hire_date)
		// xlsx.SetCellValue(sheet1Name, fmt.Sprintf("E%d", each.id), each.address_jalan)

		// var ext = each.hire_date
		// if ext == null {
		// 	xlsx.SetCellValue(sheet1Name, fmt.Sprintf("D%d", each.id), "each.hire_date")
